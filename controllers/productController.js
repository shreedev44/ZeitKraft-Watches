...
const loadProducts = async (req, res) => {
    try{
        ...
        res.render('products', {products: products, search: search, name: req.session.admin});
    }
    catch(err){
...
const loadAddProduct = async (req, res) => {
    try{
        ...
        res.render('addProduct', {categories: categories, brands: brands, name: req.session.admin});
    }
    catch (err) {
        console.log(err.message);
    }
...
const loadEditProduct = async (req, res) => {
    try{
        ...
        const categories = await Category.find();
        const brands = await Brand.find();
        res.render('editProduct', {product: product, categories: categories, brands: brands, name: req.session.admin});
    }
    catch (err) {
        console.log(err.message);
    }
...
const editProduct = async (req, res) => {
    try{
        ...
        if('productPic1' in body){
            const imagePath = path.join(__dirname, "..", "public", "uploads", "products", oldImage1);
            ...
            const imagePath = path.join(__dirname, "..", "public", "uploads", "products", oldImage2);
            fs.unlink(imagePath, (err) => {
                if (err) {
                    console.log("Error deleting old image:", err);
                }
            ...
            const imagePath = path.join(__dirname, "..", "public", "uploads", "products", oldImage3);
            fs.unlink(imagePath, (err) => {
                if (err) {
                    console.log("Error deleting old image:", err);
                }
