    ...
    if (entityOf == "category") {
      ...
      entities = await Product.aggregate([
        ...
        { $unwind: "$brand" },
        {
          $project: {
            name: {
              $concat: ["$brand.brandName", " ", "$productName"],
            },
          },
        },
      ]);
...
const addOffer = async (req, res) => {
  try {
    const { offerPercent, categoryId, productId, brandId } = req.body;

    if (categoryId) {
      await Category.findByIdAndUpdate(categoryId, {
        $set: { offerPercent: offerPercent },
      });
    } else if (brandId) {
...
const loadEditOffer = async (req, res) => {
  try {
    ...
    res.render("editOffer", { offer: offer, name: req.session.admin });
  } catch (err) {
    console.log(err);
  }
};
...
const editOffer = async (req, res) => {
  try {
    await Offer.findByIdAndUpdate(req.query.offerId, req.body);
    res.sendStatus(200);
  } catch (err) {
    console.log(err);
    res.sendStatus(500);
...
const deleteOffer = async (req, res) => {
  try {
    ...
    if (offer.offerType == "Category Offer") {
      ...
      await Brand.findByIdAndUpdate(offer.brandId, {
        $unset: { offerPercent: "" },
      });
    } else {
      return res.sendStatus(404);
